...
import { getEpigramDetail, updateEpigram } from "@/api/epigram";
import Input from "@/components/input";
import { useAuth } from "@/contexts/AuthContext";
import { buildUpdateEpigramBody } from "@/lib/build-epigram-body";
...
): { type: AuthorType; customName: string } {
  if (author === UNKNOWN_AUTHOR_LABEL) {
    return { type: "unknown", customName: "" };
  }
  if (userName && author === userName) {
    return { type: "self", customName: "" };
  }
  return { type: "custom", customName: author };
}

export default function EditEpigramForm({ epigramId }: EditEpigramFormProps) {
  ...
  useEffect(() => {
    ...
    const loadEpigram = async () => {
      try {
        const epigram = await getEpigramDetail(epigramId);

        if (cancelled) return;

        if (userId !== null && epigram.writerId !== userId) {
          router.replace(`/epigrams/${epigramId}`);
          return;
        }

        const inferred = inferAuthorType(epigram.author, userName);
        setContent(epigram.content);
        setAuthorType(inferred.type);
        setAuthorName(inferred.customName);
        setReferenceTitle(epigram.referenceTitle ?? "");
        setReferenceUrl(epigram.referenceUrl ?? "");
        setTags((epigram.tags ?? []).map((tag) => tag.name));
      } catch {
        if (!cancelled) {
          setLoadError("에피그램을 불러오지 못했습니다.");
        }
      } finally {
        if (!cancelled) {
          setIsLoading(false);
  ...
  }, [epigramId, isLoggedIn, router, userId, userName]);
  ...
  if (isLoading) {
    return <LoadingMessage>에피그램을 불러오는 중...</LoadingMessage>;
  }

  if (loadError) {
    return <FormError role="alert">{loadError}</FormError>;
  }
